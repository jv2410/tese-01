...
export default function DocumentPage() {
  ...
  const loadSettings = async () => {
    try {
      ...
      setSelectedModels({
        openai: data.settings.models.openai[0] || '',
        gemini: data.settings.models.gemini[0] || '',
        grok: data.settings.models.grok[0] || ''
      });
    } catch (error: any) {
  ...
  const handleChat = async (actionType: string | null = null) => {
    ...
    try {
      ...
      const models: any = {};
      providers.forEach((p) => {
        models[p] = selectedModels[p as keyof typeof selectedModels];
      });
